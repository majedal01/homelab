...
export default async function DashboardPage({
  ...
  // 12-month window for the trend chart, ending in the current month.
  const trendMonths = monthsWindow(now, 12);
  const trendFrom = monthBounds(trendMonths[0]).from;
  const trendTo = thisMonthBounds.to;

  const [accounts, recentTxns, thisMonthTxns, lastMonthTxns, trendTxns, donutTxns] =
    await Promise.all([
  ...
  // Net worth doesn't have a clean "previous month" comparable without a
  // balance-history table. Show no delta for now.
  ...
  const trendPoints = monthlyTrend(trendMonths, trendTxns, onBudgetIds);
